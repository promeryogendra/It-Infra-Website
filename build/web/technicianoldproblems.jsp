<%@page import="itinfra.problemlist"%>
...
#cancel:hover{
    ...
    <body>
		...
		<h1 class="sub_head">Old Problems</h1>
			...
			<table border="1" class="table-fill">
				<thead>
					<tr>
						...
						<th class="text-left2" height="5" width="300">ID NO</th>
						<th class="text-left3" height="5" width="300">PROBLEM TYPE</th>
						<th class="text-left3" height="5" width="300">Description</th>
						<th class="text-left3" height="5" width="300">SLOT</th>
						<th class="text-left3" height="5" width="300">STATUS</th>
						<th class="text-left4" height="5" width="300">VIEW</th>
					</tr>
				</thead>
				<tbody class="table-hover">
				<%int i=1;
				technicianmethods a=new technicianmethods();
				List list=a.getoldproblems();
				Iterator<problemlist> itr=list.iterator();
				while(itr.hasNext())
					{
					problemlist u=itr.next();
					String col="green";
					if(u.getPstatus().equals("inprogress"))
						col="blue";
					%>
						<tr style="width:100%;height: 10px;">
							<TD class="text-left1" ><%=i++%></TD>
							<TD class="text-left2" ><%=u.getSid()%></TD>
							<TD class="text-left3"><%=u.getPtypestring()%></TD>
							<TD class="text-left3"><%=u.getPdesc()%></TD>
							<TD class="text-left3"><%=u.getSlot() %></TD>
							<TD class="text-left3" style="color:<%=col%>"><%=u.getPstatus()%></TD>
							<TD id="cancel" class="text-left4">
								<form action="technicianviewoldproblem.jsp" method="post">
									<input type="hidden" name="sid" value="<%=u.getSid()%>"/>
									<input type="hidden" name="ptype" value="<%=u.getPtype()%>"/>
									<input type="hidden" name="pid" value="<%=u.getPid()%>"/>
									<input type="hidden" name="pdesc" value="<%=u.getPdesc()%>"/>
									<input type="hidden" name="ptypestring" value="<%=u.getPtypestring()%>"/>
									<input type="hidden" name="pstatus" value="<%=u.getPstatus()%>"/>
									<input type="hidden" name="time" value="<%=u.getSlot()%>"/>
									<input  type="image" src="images/open5.png" name="remove" style="margin-left:60px;width:50px;height:50px;"/>
								</form>
							</TD>
