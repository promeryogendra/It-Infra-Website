	...
	*{
    ...
    <body>
		...
		<div class="topnav">
			...
			<a class="active"  href="studentnewproblem.jsp">New Problem</a>
			<a href="studentprofile.jsp">Profile</a>
		</div>
		<h1 class="sub_head">New Problem</h1>
		<%
		...
		<div class="body_space">
			<form method="post" action="studentnewproblem">
				<label for="user" class="label">Problem Type</label>
				<div class="select-field">
					<select name="ptype" required >
						  <option disabled selected>Select problem Type</option>
							<%while(itr.hasNext())
							{
							String pt=(String)itr.next();
							%>
								 <option value="<%=pt%>"><%=i+++"). "+pt%></option>
							<%}%>
					</select>
				</div>
				<label for="user" style="margin-top:100px;" class="label">Problem Description</label>
				<div class="textarea_class">
					<textarea maxlength="200" class="textarea" name="pdesc" cols="39" rows="4" placeholder="Enter the description with in 200 letters" required pattern=".{255}"></textarea>
				</div>
				<input style="font-size:20px;" class="post" type="submit" value="Post">
			</form>
			<%
				String createerror,color="red";
				createerror=(String)session.getAttribute("createerror");
				if(createerror==null)
				{
					createerror="Please Fill All Fileds....";
					color="green";
				}
				else if(createerror.equals("Problem posted successfully...."))
				{
					color="green";
				}
				session.setAttribute("createerror",null);
			%>
			<p style="text-align:center;color:<%=color%>;text-size:26px;margin-top:375px;margin-right:110px;" text=" "> <%=createerror%></p>
		</div>
